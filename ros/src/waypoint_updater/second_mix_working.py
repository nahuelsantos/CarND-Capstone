...
class WaypointUpdater(object):
    ...
    def loop(self):
        next_waypoints = []
        check_skip = [self.base_waypoints, self.last_pose]
        if None not in check_skip:
            self.closest_waypoint = self.nearest_wp(self.last_pose.pose, self.base_waypoints)
    ...
    def nearest_wp(self, last_pose, waypoints):
        ...
        for index, waypoint in enumerate(waypoints):
            ...
            distance = dl(last_pose.position, waypoint_pos)
            if distance < nearest_distance:
                nearest_index = index
                nearest_distance = distance
        
        nearest_index = self.adjust_closest_wpt(nearest_index, waypoints, last_pose)
